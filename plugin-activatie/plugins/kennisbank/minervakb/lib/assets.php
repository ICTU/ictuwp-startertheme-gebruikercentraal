...
class MinervaKB_Assets {
	...
	public function client_assets() {
		...
		if ( MKB_Options::option( 'article_fancybox' ) && is_single() && $post->post_type == MKB_Options::option( 'article_cpt' ) ) {
			...
			wp_enqueue_script( 'minerva-kb/fancybox-js', MINERVA_KB_PLUGIN_URL . 'assets/js/vendor/fancybox/jquery.fancybox-1.3.4.js', array(
				'jquery',
				'minerva-kb/fancybox-easing-js'
			), '1.3.4.1', true );
		}
	...
	public function admin_block_editor_page_assets() {
		 ...
		 */
		if ( isset( $screen ) && $screen->id == 'page' && $screen->post_type == 'page' ) {
			wp_enqueue_script( 'minerva-kb/admin-page-blocks-js', MINERVA_KB_PLUGIN_URL . 'assets/js/minerva-kb-page-gutenberg.js', array(), MINERVA_KB_VERSION, true );
		}
	}
}
