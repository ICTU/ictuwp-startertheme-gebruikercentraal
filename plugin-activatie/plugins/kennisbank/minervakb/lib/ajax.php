...
class MinervaKB_Ajax {
	...
	public function article_feedback() {
		...
		if (MKB_Options::option('email_notify_feedback_switch')) {
            $to = MKB_Options::option('email_notify_default_email');
            $subject = MKB_Options::option('email_notify_feedback_subject');

            ob_start();

            ?>
            <p><?php echo MKB_Options::option('email_notify_feedback_message'); ?> <strong>
                <a href="<?php esc_attr_e(esc_url(get_permalink($article_id))); ?>" target="_blank">
                    <?php esc_html_e(get_the_title($article_id)); ?>
                </a></strong>
            </p>
            <blockquote style="background: #f7f7f7;color: #333;margin: 1em 0;padding: 1em 2em;font-size: 1.1em;">
                <?php echo wpautop(stripslashes($feedback_content)); ?>
            </blockquote>
            <?php
...
// TODO: remove after 1.6.4 release
