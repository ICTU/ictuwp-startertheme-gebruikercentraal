...
class GebruikerCentraalTheme extends Timber\Site {
  ...
  public function add_to_context($context) {
    ...
    if (is_page() && !empty(get_the_id())) {
      if (function_exists('has_blocks') && has_blocks(get_the_id())) {
        $context['is_gutenberg'] = TRUE;
      } else {
        $context['is_gutenberg'] = FALSE;
      }
    }
...
function insert_breadcrumb() {
  ...
  if (function_exists('yoast_breadcrumb')) {
    ...
    return  yoast_breadcrumb($html_start, $html_end, TRUE);
  }
}

function filter_wpseo_breadcrumb_single_link( $link_output, $link ) {
  // make filter magic happen here...
  return '<a class="breadcrumb__link" href="'. $link['url'].'">'. $link['text'] . '</a>';
};
...
add_filter( 'wpseo_breadcrumb_separator', function( $separator ) {
  return '<span class="breadcrumb__separator">&raquo;</span>';
} );
...
function size_column_display($column_name, $post_id) {
  ...
  [$url, $width, $height] = wp_get_attachment_image_src($post_id, 'full');
